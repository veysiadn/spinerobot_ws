...
int EthercatNode::MapDefaultPdosForMotors()
{
    for(int i = 0 ; i < g_kNumberOfServoDrivers -1 ; i++){    
        int err = ecrt_slave_config_pdos(this->slaves_[i].slave_config_,EC_END,this->slaves_[i].elmo_syncs);
        if ( err ) {
...
int EthercatNode::MapCustomPdos(EthercatSlave S, int position)
{
        int err = ecrt_slave_config_pdos(S.slave_config_,EC_END,S.slave_syncs_);
        if ( err ) {
            RCLCPP_ERROR(rclcpp::get_logger(__PRETTY_FUNCTION__), "Failed to configure  PDOs!  ");
            return -1;
        } 
        err = ecrt_domain_reg_pdo_entry_list(g_master_domain, S.slave_pdo_regs_);
        if ( err ){
            RCLCPP_ERROR(rclcpp::get_logger(__PRETTY_FUNCTION__), "Failed to register PDO entries ");
            return -1;
        }
